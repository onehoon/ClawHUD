...
int main()
{
    ...
    ok &= Check(Near(geometry.textOrigin.x, 311.0f), "center text origin");
    ...
    ok &= Check(Near(geometry.background.left, 0.0f) &&
        Near(geometry.background.right, 400.0f) && Near(geometry.textOrigin.x, 11.0f),
        "left ContentWidth geometry");
    options.layout.alignment = HudAlignment::Right;
    geometry = CalculateHudGeometry(viewport, measure, options);
    ok &= Check(Near(geometry.background.left, 600.0f) &&
        Near(geometry.background.right, 1000.0f) && Near(geometry.textOrigin.x, 611.0f),
    ...
    ok &= Check(Near(geometry.textOrigin.x, 7.3333f), "144 DPI physical padding");
    ...
    ok &= Check(Near(MainTextYOffset(typographyOptions), 0.0f) &&
        Near(UnitTextYOffset(typographyOptions), 4.0f),
    ...
    typographyOptions.font = HudFont::SegoeUiVariable;
    ok &= Check(Near(MainTextYOffset(typographyOptions), 0.0f) &&
        Near(UnitTextYOffset(typographyOptions), 2.6667f),
        "Segoe UI Variable unit optical offset is 4 physical px at high DPI");
