    ...
    std::map<std::string, IgclSampleSeries>& metrics)
{
    ...
    auto sampleFull=[&](const char* ename,const char* sname,const char* metric){
        ...
        for(std::uint32_t i=0;i<n;++i){const std::string index=std::string(metric)+"["+std::to_string(i)+"]";
            if(std::string(metric)=="frequency"){igcl::FrequencyState v{};v.Size=sizeof(v);v.Version=1;const Result r=st(hs[i],&v);fail(index,true,true,r==kSuccess);if(r==kSuccess){collect(index+".currentVoltage",v.currentVoltage,0,9,true);collect(index+".request",v.request,0,9,true);collect(index+".tdp",v.tdp,0,9,true);collect(index+".efficient",v.efficient,0,9,true);collect(index+".actual",v.actual,0,9,true);collect(index+".throttleReasons",static_cast<double>(v.throttleReasons),v.throttleReasons,7,true);}}
            else if(std::string(metric)=="engine"){igcl::EngineStats v{};v.Size=sizeof(v);v.Version=1;const Result r=st(hs[i],&v);fail(index,true,true,r==kSuccess);if(r==kSuccess){collect(index+".activeTime",static_cast<double>(v.activeTime),v.activeTime,7,true);collect(index+".timestamp",static_cast<double>(v.timestamp),v.timestamp,7,true);}}
            else if(std::string(metric)=="memory"){igcl::MemoryState v{};v.Size=sizeof(v);v.Version=1;const Result r=st(hs[i],&v);fail(index,true,true,r==kSuccess);if(r==kSuccess){collect(index+".free",static_cast<double>(v.free),v.free,7,true);collect(index+".size",static_cast<double>(v.size),v.size,7,true);}auto bw=Get<OneStateFn>(lib,"ctlMemoryGetBandwidth");if(!bw){fail(index+".bandwidth",false,true,true);}else{igcl::MemoryBandwidth b{};b.Size=sizeof(b);b.Version=1;const Result br=bw(hs[i],&b);fail(index+".bandwidth",true,true,br==kSuccess);if(br==kSuccess){collect(index+".bandwidth.maxBandwidth",static_cast<double>(b.maxBandwidth),b.maxBandwidth,7,true);collect(index+".bandwidth.timestamp",static_cast<double>(b.timestamp),b.timestamp,7,true);collect(index+".bandwidth.readCounter",static_cast<double>(b.readCounter),b.readCounter,7,true);collect(index+".bandwidth.writeCounter",static_cast<double>(b.writeCounter),b.writeCounter,7,true);}}}
            else if(std::string(metric)=="power"){igcl::PowerEnergy v{};v.Size=sizeof(v);v.Version=1;const Result r=st(hs[i],&v);fail(index,true,true,r==kSuccess);if(r==kSuccess){collect(index+".energy",static_cast<double>(v.energy),v.energy,7,true);collect(index+".timestamp",static_cast<double>(v.timestamp),v.timestamp,7,true);}}
        }
    };
    ...
    try {
        ...
        for (int sample=1; sample<=20 && !stop_; ++sample)
        {
            ...
            for (std::uint32_t i=0;i<count;++i)
            {
                ...
                PowerV2 p{}; p.size=sizeof(p); p.version=1; const Result sr=tele ? tele(ds[i],&p) : 0x40000000u;
                if (tele && sr == kSuccess)
                {
