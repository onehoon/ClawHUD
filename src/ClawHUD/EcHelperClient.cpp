...
namespace
{
const wchar_t* FailureStageName(clawhud::ec::EcFailureStage stage) noexcept
{
    using clawhud::ec::EcFailureStage;
    switch (stage)
    {
    case EcFailureStage::HelperMissing: return L"HelperMissing";
    case EcFailureStage::HelperLaunch: return L"HelperLaunch";
    case EcFailureStage::HelperNotElevated: return L"HelperNotElevated";
    case EcFailureStage::InvalidResponse: return L"InvalidResponse";
    case EcFailureStage::InvalidSuccessFlag: return L"InvalidSuccessFlag";
    case EcFailureStage::Pipe: return L"Pipe";
...
bool EcHelperClient::EnsureConnected()
{
    ...
    if (!StartHelper(pipeName_)) { CloseTransport(); return false; }

    OVERLAPPED overlap{}; overlap.hEvent = CreateEventW(nullptr, TRUE, FALSE, nullptr);
    if (!overlap.hEvent) { Failure(HRESULT_FROM_WIN32(GetLastError()), clawhud::ec::EcFailureStage::Pipe); CloseTransport(); return false; }
    ...
    if (!connected && connectError != ERROR_IO_PENDING && connectError != ERROR_PIPE_CONNECTED)
    {
        ...
        CloseHandle(overlap.hEvent); CloseTransport(); return false;
    }
...
bool EcHelperClient::StartHelper(const std::wstring& pipeName)
{
    ...
    ++g_startHelperCalls;
    if (g_forceStartHelperFailure)
    {
        Failure(HRESULT_FROM_WIN32(ERROR_CANCELLED), clawhud::ec::EcFailureStage::HelperLaunch);
        return false;
    }
#endif
    ...
    std::vector<std::uint8_t>& payload)
{
    ...
    if (!WriteAll(&request, sizeof(request))) { CloseTransport(); return false; }
    clawhud::ec::EcResponse response{};
    if (!ReadAll(&response, sizeof(response))) { CloseTransport(); return false; }
    ...
    if (!response.success)
    {
        if (stage_ == clawhud::ec::EcFailureStage::None)
            stage_ = clawhud::ec::EcFailureStage::InvalidSuccessFlag;
...
void EcHelperClient::CloseTransport()
{
...
{
    CloseTransport();
    // Explicit lifetime release: only here does a consumed launch attempt become
    // eligible again, so a later explicit HUD re-enable can request elevation.
    attempted_ = false;
}
