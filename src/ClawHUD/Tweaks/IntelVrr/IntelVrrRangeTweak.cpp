...
IntelVrrRunResult IntelVrrRangeTweak::Run(bool enabled)
{
    ...
    IntelArcSyncProfileState verify{}; if (!client->GetArcSyncProfile(selected.first, verify)) { result.status = IntelVrrRunStatus::VerificationFailed; result.message = "Applied the profile but could not verify it took effect."; result.rangeAfter = "unknown"; return finish(result); } flush(); result.rangeAfter = Range(verify.minimumHz, verify.maximumHz);
