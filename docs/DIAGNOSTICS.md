...
PresentMon starts once for the complete diagnostic and records one CSV in the canonical log directory. It samples 28 seconds in each phase with a one-second transition interval excluded between phases. QPC phase ranges are recorded in the TXT report and the same CSV is filtered by those ranges. No `ClawHUD.VrrPoc.exe` child, test overlay, or second presentation path is created; the prior main-HUD visibility state is restored after completion, failure, or cancellation.
