...
Logs are UTF-8 text files under `logs/diagnostics/ec-YYYYMMDD-HHMMSS.txt`. **Open Log Folder** opens that directory. The log includes elevation state and records `Unavailable` for environment fields that cannot be obtained. A read failure for one selector does not stop the remaining selectors; WMI connection failure ends the test and is recorded. EC failures include the native HRESULT and transport stage, such as `Put Data.Bytes`, `ExecMethod`, `Extract output.Data.Bytes`, or `Validate response[0]`, so an administrator run can distinguish request construction from device response failures.
