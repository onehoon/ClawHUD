...
Settings is created only when selected. It contains the General, HUD, Tweaks, and Diagnostics tabs. The Diagnostics tab exposes only the debug-logging toggle and **Open Log Folder**; there is no in-app developer diagnostic. The Tweaks tab only persists the Intel VRR Range Fix startup toggle and displays its last result; it never applies a profile from the UI. The Settings minimize and close buttons hide the window to the tray while keeping ClawHUD running; selecting Settings again restores the same window.
