    ...
    const char* enumName, const char* propsName, const char* stateName)
{
    ...
    for(std::uint32_t i=0;i<count;++i)
    {
        ...
        else if (std::string(title) == "ENGINE") { igcl::EngineProperties p{};p.Size=sizeof(p);auto f=Get<igcl::PropsFn>(lib,propsName);if(f){r=f(handles[i],&p);LogCall(o,propsName,r);if(r==0)o<<"EngineType: "<<p.type<<"\n";}igcl::EngineStats s{};s.Size=sizeof(s);auto g=Get<igcl::StateFn>(lib,stateName);if(g){r=g(handles[i],&s);LogCall(o,stateName,r);if(r==0)o<<"ActiveTimeUs: "<<s.activeTime<<"\nTimestampUs: "<<s.timestamp<<"\n";} }
        ...
        else if (std::string(title) == "TEMPERATURE") { igcl::TemperatureProperties p{};p.Size=sizeof(p);auto f=Get<igcl::PropsFn>(lib,propsName);if(f){r=f(handles[i],&p);LogCall(o,propsName,r);if(r==0)o<<"SensorType: "<<p.type<<"\nMaxC: "<<p.maxTemperature<<"\n";}auto g=reinterpret_cast<Result(__cdecl*)(void*,double*)>(GetProcAddress(lib,stateName));if(g){double temperature=0;r=g(handles[i],&temperature);LogCall(o,stateName,r);if(r==0)o<<"CurrentC: "<<temperature<<"\n";} }
        else if (std::string(title) == "POWER") { igcl::PowerProperties p{};p.Size=sizeof(p);auto f=Get<igcl::PropsFn>(lib,propsName);if(f){r=f(handles[i],&p);LogCall(o,propsName,r);if(r==0)o<<"CanControl: "<<(p.canControl?"true":"false")<<"\nDefaultLimitMilliwatts: "<<p.defaultLimit<<"\nMinLimitMilliwatts: "<<p.minLimit<<"\nMaxLimitMilliwatts: "<<p.maxLimit<<"\n";}igcl::PowerEnergy e{};e.Size=sizeof(e);auto g=Get<igcl::StateFn>(lib,stateName);if(g){r=g(handles[i],&e);LogCall(o,stateName,r);if(r==0)o<<"EnergyMicrojoules: "<<e.energy<<"\nTimestampUs: "<<e.timestamp<<"\n";} }
        else if (std::string(title) == "FAN") { igcl::FanProperties p{};p.Size=sizeof(p);auto f=Get<igcl::PropsFn>(lib,propsName);if(f){r=f(handles[i],&p);LogCall(o,propsName,r);if(r==0)o<<"CanControl: "<<(p.canControl?"true":"false")<<"\nSupportedModes: "<<p.supportedModes<<"\nSupportedUnits: "<<p.supportedUnits<<"\nMaxRPM: "<<p.maxRPM<<"\n";}auto g=Get<igcl::FanStateFn>(lib,"ctlFanGetState");if(g){std::int32_t speed=-1;r=g(handles[i],0,&speed);LogCall(o,"ctlFanGetState",r);if(r==0)o<<"SpeedRaw: "<<speed<<"\n";} }
    ...
    std::map<std::string, IgclSampleSeries>& metrics)
{
    ...
    auto markCategoryLeaves=[&](const std::string& metric,bool symbol,bool domain,bool api){const std::vector<std::string> suffixes=metric=="frequency"?std::vector<std::string>{"actual"}:metric=="engine"?std::vector<std::string>{"activeTime"}:metric=="memory"?std::vector<std::string>{"free","size","bandwidth.maxBandwidth","bandwidth.timestamp","bandwidth.readCounter","bandwidth.writeCounter"}:metric=="power"?std::vector<std::string>{"energy","timestamp"}:metric=="temperature"?std::vector<std::string>{"current"}:std::vector<std::string>{"speed"};for(const auto& suffix:suffixes)RecordIgclDynamicLeaf(metrics[key(metric+"."+suffix)],symbol,domain,api,0,0,0);};
    auto collect=[&](const std::string& name,double value,std::uint64_t raw,std::uint32_t type,bool ok){auto& s=metrics[key(name)];const auto separator=name.rfind('.');if(separator!=std::string::npos){const auto& parent=metrics[key(name.substr(0,separator))];s.symbolPresent&=parent.symbolPresent;s.hasDomain&=parent.hasDomain;}RecordIgclDynamicLeaf(s,true,true,ok,value,raw,type);};
    ...
    auto sampleFull=[&](const char* ename,const char* sname,const char* metric){
        ...
        if(!en||!st){fail(metric,en&&st,true,true);markCategoryLeaves(metric,en&&st,true,true);return;}
        std::uint32_t n{}; const Result cr=en(device,&n,nullptr);
        if(cr!=kSuccess){fail(metric,true,true,false);markCategoryLeaves(metric,true,true,false);return;} if(!n){fail(metric,true,false,true);markCategoryLeaves(metric,true,false,true);return;}
        std::vector<Handle> hs(n); const Result er=en(device,&n,hs.data()); if(er!=kSuccess){fail(metric,true,true,false);markCategoryLeaves(metric,true,true,false);return;}
        for(std::uint32_t i=0;i<n;++i){const std::string index=std::string(metric)+"["+std::to_string(i)+"]";
            if(std::string(metric)=="frequency"){igcl::FrequencyState v{};v.Size=sizeof(v);v.Version=igcl::kLegacyStateVersion;const Result r=st(hs[i],&v);fail(index,true,true,r==kSuccess);collect(index+".currentVoltage",v.currentVoltage,0,9,r==kSuccess);collect(index+".request",v.request,0,9,r==kSuccess);collect(index+".tdp",v.tdp,0,9,r==kSuccess);collect(index+".efficient",v.efficient,0,9,r==kSuccess);collect(index+".actual",v.actual,0,9,r==kSuccess);collect(index+".throttleReasons",static_cast<double>(v.throttleReasons),v.throttleReasons,7,r==kSuccess);}
            else if(std::string(metric)=="engine"){igcl::EngineStats v{};v.Size=sizeof(v);v.Version=igcl::kLegacyStateVersion;const Result r=st(hs[i],&v);fail(index,true,true,r==kSuccess);collect(index+".activeTime",static_cast<double>(v.activeTime),v.activeTime,7,r==kSuccess);collect(index+".timestamp",static_cast<double>(v.timestamp),v.timestamp,7,r==kSuccess);}
            else if(std::string(metric)=="memory"){igcl::MemoryState v{};v.Size=sizeof(v);v.Version=igcl::kLegacyStateVersion;const Result r=st(hs[i],&v);fail(index,true,true,r==kSuccess);collect(index+".free",static_cast<double>(v.free),v.free,7,r==kSuccess);collect(index+".size",static_cast<double>(v.size),v.size,7,r==kSuccess);auto bw=Get<OneStateFn>(lib,"ctlMemoryGetBandwidth");if(!bw){for(const auto& suffix:{"maxBandwidth","timestamp","readCounter","writeCounter"})RecordIgclDynamicLeaf(metrics[key(index+".bandwidth."+suffix)],false,true,true,0,0,0);fail(index+".bandwidth",false,true,true);}else{igcl::MemoryBandwidth b{};b.Size=sizeof(b);b.Version=igcl::kTelemetryVersion;const Result br=bw(hs[i],&b);fail(index+".bandwidth",true,true,br==kSuccess);collect(index+".bandwidth.maxBandwidth",static_cast<double>(b.maxBandwidth),b.maxBandwidth,7,br==kSuccess);collect(index+".bandwidth.timestamp",static_cast<double>(b.timestamp),b.timestamp,7,br==kSuccess);collect(index+".bandwidth.readCounter",static_cast<double>(b.readCounter),b.readCounter,7,br==kSuccess);collect(index+".bandwidth.writeCounter",static_cast<double>(b.writeCounter),b.writeCounter,7,br==kSuccess);}}
            else if(std::string(metric)=="power"){igcl::PowerEnergy v{};v.Size=sizeof(v);v.Version=igcl::kLegacyStateVersion;const Result r=st(hs[i],&v);fail(index,true,true,r==kSuccess);collect(index+".energy",static_cast<double>(v.energy),v.energy,7,r==kSuccess);collect(index+".timestamp",static_cast<double>(v.timestamp),v.timestamp,7,r==kSuccess);}
        }
    };
    ...
    auto temps=Get<EnumFn>(lib,"ctlEnumTemperatureSensors");auto tempState=Get<OneStateFn>(lib,"ctlTemperatureGetState");if(!temps||!tempState){fail("temperature",temps&&tempState,true,true);markCategoryLeaves("temperature",temps&&tempState,true,true);}else{std::uint32_t n{};const Result r=temps(device,&n,nullptr);if(r!=kSuccess){fail("temperature",true,true,false);markCategoryLeaves("temperature",true,true,false);}else if(!n){fail("temperature",true,false,true);markCategoryLeaves("temperature",true,false,true);}else{std::vector<Handle> hs(n);if(temps(device,&n,hs.data())!=kSuccess){fail("temperature",true,true,false);markCategoryLeaves("temperature",true,true,false);}else for(std::uint32_t i=0;i<n;++i){double value{};const Result tr=reinterpret_cast<Result(__cdecl*)(void*,double*)>(tempState)(hs[i],&value);fail("temperature["+std::to_string(i)+"].current",true,true,tr==kSuccess);collect("temperature["+std::to_string(i)+"].current",value,static_cast<std::uint64_t>(value),9,tr==kSuccess);}}}
    auto fans=Get<EnumFn>(lib,"ctlEnumFans");auto fanState=Get<FanStateFn>(lib,"ctlFanGetState");if(!fans||!fanState){fail("fan",fans&&fanState,true,true);markCategoryLeaves("fan",fans&&fanState,true,true);}else{std::uint32_t n{};const Result r=fans(device,&n,nullptr);if(r!=kSuccess){fail("fan",true,true,false);markCategoryLeaves("fan",true,true,false);}else if(!n){fail("fan",true,false,true);markCategoryLeaves("fan",true,false,true);}else{std::vector<Handle> hs(n);if(fans(device,&n,hs.data())!=kSuccess){fail("fan",true,true,false);markCategoryLeaves("fan",true,true,false);}else for(std::uint32_t i=0;i<n;++i){std::int32_t speed=-1;const Result fr=fanState(hs[i],0,&speed);fail("fan["+std::to_string(i)+"].speed",true,true,fr==kSuccess);collect("fan["+std::to_string(i)+"].speed",static_cast<double>(speed),static_cast<std::uint64_t>(speed),4,fr==kSuccess);}}}
}
...
bool IgclTelemetryDiagnostic::Start(){ if(running_.exchange(true))return false; stop_=false; try{worker_=new std::thread(&IgclTelemetryDiagnostic::Run,this);}catch(...){running_=false;delete worker_;worker_=nullptr;return false;} Status(L"Waiting 5 seconds..."); return true; }
...
void IgclTelemetryDiagnostic::Run()
{
    ...
    try {
        for(int i=0;i<50&&!stop_.load();++i)std::this_thread::sleep_for(std::chrono::milliseconds(100)); if(stop_) { Status(L"Cancelled"); throw std::runtime_error("cancelled"); }
        ...
        for(std::uint32_t i=0;i<count;++i)
        {
            ...
            if (const auto live = Get<GetSet3DFn>(library, "ctlGetSet3DFeature"); live)
            {
                ...
                else
                {
                    ...
                    const Result lr=live(ds[i],&request); LogCall(log,"ctlGetSet3DFeature (read-only)",lr); if(lr==kSuccess) log<<"Graphics API mask raw: 0x"<<std::hex<<state.gfxApi<<"\nTarget FPS raw: "<<std::dec<<state.targetFps<<"\nFrame pacing raw: "<<state.framePacingStatus<<"\n"; else log<<"3D Live State values: API_ERROR\n";
                }
            }
